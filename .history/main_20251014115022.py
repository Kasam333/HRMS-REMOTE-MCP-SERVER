...
def get_employee_details(employee_id: int) -> dict:
    ...
    cur = conn.cursor()
    cur.execute("""
                SELECT e.id, e.serial_number, e.name, e.job_title, e.work_email, e.work_phone,
                    d.name AS department, c.name AS company, e.work_location, e.parent_id
                FROM hr_employee e
                LEFT JOIN hr_department d ON e.department_id
