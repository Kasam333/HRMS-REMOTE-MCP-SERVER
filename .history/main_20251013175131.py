...
def get_user_groups(employee_id: int) -> list[str]:
    ...
    try:
        ...
        cur.execute("""
                    ...
                    WHERE r.uid = %s
                    """, (user_id,))
        groups = [row[0].strip().lower() for row in cur.fetchall()]
        return groups
...
def list_employees(employee_id: int) -> list[dict]:
    ...
    groups = get_user_groups(employee_id)
    if not groups:
        return [{"error": "Invalid employee ID or user not linked."}]
    
    allowed_groups = {"group_attendaadminnce_admin"}
